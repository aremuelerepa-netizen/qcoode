...
# ─── SCHEMA COLUMN CACHE ──────────────────────────────────────
# Prevents inserting columns that don't exist yet in Supabase
_schema_cache: dict = {}
...
def _safe_payload(table: str, required: dict, optional: dict) -> dict:
    ...
    Optional fields only included if the column exists in the table schema.
    This prevents 400 errors when new columns haven't been added to Supabase yet.
    """
...
def send_reset_email(to_email, reset_link, is_org=False):
    ...
    if not SENDGRID_API_KEY:
        print(f"[Reset Email] To: {to_email}\nLink: {reset_link}")
        return True
    try:
        r = requests.post("https://api.sendgrid.com/v3/mail/send",
            ...
            json={
                ...
                "content": [{"type": "text/html", "value":
                    ...
                    <a href="{reset_link}" style="display:inline-block;background:#4361ee;color:#fff;
                       padding:.875rem 2rem;border-radius:.75rem;text-decoration:none;font-weight:700;margin:1rem 0">
                       Reset Password &rarr;</a>
                    <p style="color:#8892a4;font-size:.85rem">If you didn't request this, ignore this email.</p>
                    </div>"""
                }]
            }, timeout=15)
        return r.ok
...
def api_me():
    ...
    return jsonify({"logged_in": True, "user_id": session["user_id"],
                    ...
                    "full_name": session.get("full_name"), "org_name": session.get("org_name")}), 200
...
def login_org():
    ...
    if role != "organization":
        sb_signout(token); return jsonify({"error": "This page is for organizations only."}), 403
    if status == "pending":
        sb_signout(token); return jsonify({"error": "Your organization is pending admin approval."}), 403
    if status == "suspended":
        sb_signout(token); return jsonify({"error": "This account has been suspended."}), 403
    session.permanent = True
...
def forgot_password():
    ...
    if profiles:
        ...
        if (is_org and role == "organization") or (not is_org and role == "user"):
            ...
            send_reset_email(email, link, is_org=is_org)
...
def api_join_queue():
    ...
    entry.update({"position": pos, "svc_name": svc["name"], "time_interval": svc.get("time_interval",5),
                  "end_code": end_code,
                  "org_name": (db_select("profiles",{"id":f"eq.{svc['org_id']}"},single=True) or {}).get("org_name","")})
    send_push_to_user(uid,"✅ Joined Queue!",f"Ticket {label} — Position #{pos}",
                      {"type":"joined","entry_id":entry.get("id")})
    phone = profile.get("phone","")
    if phone:
        send_sms(phone, f"QCode: Joined {svc['name']}. Ticket: {label} | #{pos} | ~{pos*(svc.get('time_interval') or 5)} min. End code: {end_code}")
...
def api_user_history():
    ...
    for row in rows:
        ...
        if sid in cache:
            row["svc_name"] = cache[sid]["name"]
            row["org_name"] = cache[sid]["org_name"]
...
def api_auto_cancel():
    """
    Cron endpoint (~every 2 min via cron-job.org).
    Pushback system: called → push back 3x → then no_show permanently.
    """
        ...
        if pb >= 3:
            ...
            if entry.get("user_id"):
                send_push_to_user(entry["user_id"],f"⚠️ Pushed Back ({pb+1}/3)",
                                  ...
                                  {"type":"pushback","count":pb+1})
            if entry.get("guest_phone"):
                send_sms(entry["guest_phone"],
                         f"QCode: Ticket {entry.get('ticket_label','—')} missed call #{pb+1}. Pushed back 3 positions. Strike {pb+1}/3.")
...
def api_auto_schedule():
    """Cron endpoint — opens/closes services by their schedule."""
...
# ── LOGO UPLOAD (NEW / FIXED) ──────────────────────────────────
@app.route("/api/org/upload-logo", methods=["POST"])
def api_org_upload_logo():
    """
    Accepts multipart/form-data with field name 'logo'.
    Uploads to Supabase Storage bucket, saves public URL to profiles.logo_url.

    Supabase setup needed:
      1. Create bucket named 'org-logos' (or set SUPABASE_STORAGE_BUCKET env)
      2. Set bucket to Public
      3. Add storage policy: allow authenticated users to insert/update their own folder
    """
    uid = session.get("user_id")
    ...
    # Try POST first (new file), fall back to PUT (upsert)
...
def api_org_create_service():
    """
    FIXED:
    - user_info_form and break_times sent as native Python lists (not json.dumps strings)
    - _safe_payload used so new columns (stages_enabled, batch_enabled etc.) won't
      cause 400 errors if they don't exist in Supabase yet
    - detailed error message returned on failure for easier debugging
    """
    uid = session.get("user_id")
    if not uid or session.get("role") != "organization": return jsonify({"error": "Unauthorized"}), 403
    d    = request.get_json() or {}
    ...
    # Optional columns — only inserted if they exist in the schema
    optional = {
        "staff_name":      d.get("staff_name") or None,
        "description":     d.get("description") or None,
        "end_code":        end_code,
        "max_users":       int(d.get("max_users")) if d.get("max_users") else None,
        # FIXED: send as list, NOT json.dumps(list) — Supabase handles serialisation
        ...
        # New columns — may not exist in schema yet, _safe_payload will skip them
    ...
    if not res["ok"]:
        ...
        if isinstance(res["data"], dict):
            err = res["data"].get("message") or res["data"].get("hint") or err
    ...
    # Create stages if multi-stage service
            ...
            if s_res.get("ok") and s_res["data"]:
                ...
                for j, sname in enumerate(st.get("staff_names") or []):
                    db_insert("staff_counters", _safe_payload("staff_counters", {
                        "stage_id":       stage_id, "service_id": svc_id,
                        "staff_name":     sname,    "counter_number": j+1,
                        "is_active":      True,
                        "created_at":     datetime.now(timezone.utc).isoformat(),
                    }, {}))
...
def api_org_service_interval(svc_id):
    """Emergency: change time_interval on a live service."""
    uid = session.get("user_id")
    if not uid or session.get("role") != "organization": return jsonify({"error": "Unauthorized"}), 403
...
def api_org_walk_in(svc_id):
    """Add a walk-in customer (no account needed)."""
    uid = session.get("user_id")
    if not uid or session.get("role") != "organization": return jsonify({"error": "Unauthorized"}), 403
    d     = request.get_json() or {}
...
def api_org_move_ticket(svc_id):
    ...
    rows = db_select("queue_entries",{"service_id":f"eq.{svc_id}",
                                       "ticket_label":f"eq.{ticket_label}","status":"eq.waiting"})
...
def api_staff_access():
    ...
    if not stages:
        # Fallback: single-stage org — treat code as service_code
        ...
        return jsonify({"success":True,"stage_id":svc["id"],"stage_name":svc["name"],
                        ...
                        "time_interval":svc.get("time_interval",5)}), 200
    stage = stages[0]
        ...
        if rows:
            ...
            if svc_id and stage_id:
                ...
                if cur:
                    ...
                    if nxt:
                        pos = db_count("queue_entries",{"service_id":f"eq.{svc_id}","stage_id":f"eq.{nxt[0]['id']}","status":"eq.waiting"})+1
                        ...
                        new_req = {"service_id":svc_id,"user_id":entry.get("user_id"),
                                   ...
                                   "ticket_label":entry.get("ticket_label",""),"ticket_number":entry.get("ticket_number",0),
                                   "status":"waiting","estimated_time":eta,"join_method":entry.get("join_method","web"),
                        ...
                        if entry.get("user_id"):
                            send_push_to_user(entry["user_id"],f"➡️ Next: {nxt[0]['name']}",
                                              f"Enrolled in {nxt[0]['name']}. Position #{pos}.",
                                              {"type":"next_stage","stage_name":nxt[0]["name"]})
    return jsonify({"success":True}), 200
...
# ══════════════════════════════════════════════════════════════
#  GUEST APIs (kept for SMS/walk-in compatibility)
# ══════════════════════════════════════════════════════════════
